...
class ActorExceptionHandler {
    ...
    fun handleOnActorNotFound(): ResponseEntity<ErrorResponse> {
        val body = ErrorResponse(ACTOR_NOT_FOUND.name, ACTOR_NOT_FOUND.wording)
        return ResponseEntity(body, NOT_FOUND)
    }

    data class ErrorResponse(val errorCode: String, val errorReason: String)
}
